...
def predict():
    ...
    possibility_of_rain = "Yes" if best_predict_humidity > rain_threshold else "No"
...
def predictionMainloop():
    while True:
        ...
        print("The prediction is: Temp={}, Humidity={}, Rain? {}".format(predictedTemp, predictedHumid, possibility_of_rain))
        client.publish("banhbaochien/feeds/predict_temp", predictedTemp)
        client.publish("banhbaochien/feeds/predict_humid", predictedHumid)
        client.publish("banhbaochien/feeds/possibility_of_rain", possibility_of_rain)
